...
export default function TransactionTable ({transactions,getDetailHref}:Props) {
    return (
        <div className="overflow-x-auto"> {/* 横方向に伸びる */}
                ...
                <tbody>
                    ...
                    {transactions.map((t) => (
                        <tr key={t.id} >
                            ...
                            <td className="border-b border-slate-100 px-3 py-2">{t.amount}</td>
                            <td className="border-b border-slate-100 px-3 py-2">{t.title}</td>
                            <td className="border-b border-slate-100 px-3 py-2">
                                {/* underline:文字下線、underline-offset-4:下線と文字の間隔を広げる、hover:text-lime-200:マウスを乗せたとき薄ライム */}
                                <Link className="underline underline-offset-4 hover:text-lime-200" href={getDetailHref(t.id)}>詳細</Link>
                            </td>
                        </tr>
                    ))
                    }
