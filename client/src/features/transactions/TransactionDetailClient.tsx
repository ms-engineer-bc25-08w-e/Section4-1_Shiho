...
export default function TransacntioDetailClient({ id, initialTransactions }: props) {
  //詳細表示 / 編集・更新
  ...
  const handleUpdate = (update: Transaction) => {
    setTransactions(
      (prev) => prev.map((t) => (t.id === update.id ? update : t)), //idが一致するものだけ差し替え 三項演算子 条件 ? 真のときの値 : 偽のときの値
    );
  };
  return <TransactionDetail key={id} id={id} transactions={transactions} onUpdate={handleUpdate} />;
}
